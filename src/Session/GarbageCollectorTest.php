 ...
 */
class GarbageCollectorTest extends ADOdbTestCase
{
    /**
    ...
    public function testGarbageCollector(): void
    {
        ...
        $this->assertSame(
            0,
            (int)$pastCount,
            'There should be no records with past expiry times'
        );

        $SQL = "SELECT COUNT(*) 
                 FROM session_test
                WHERE expireref='FUTUREEXPIRY'";

        $futureCount = $GLOBALS['ADOdbConnection']->getOne($SQL);

        $this->assertSame(
            2,
            (int)$futureCount,
            'There should be 2 records with future expiry times'
        );
    }
}
