...
class CacheAsGetAssocTest extends ADOdbCoreSetup
{
     ...
     */
    #[DataProvider('providerTestCacheAsGetAssoc')]
    public function testCacheAsGetAssoc(
    ...
    public function testGetAssocViaRecordSet(
        ...
        foreach ($this->testFetchModes as $fetchMode => $fetchModeName) {
            ...
            if ($bindFlag) {
                $returnedRows = $this->db->cacheExecute($boundSql, $bind)->getAssoc($forceArray, $first2Cols);
            } else {
                $returnedRows = $this->db->cacheExecute($unboundSql, false)->getAssoc($forceArray, $first2Cols);
            }
     ...
     */
    public static function providerTestCacheAsGetAssoc(): array
    {
